...
fn min_max(row: &str) -> (u64, u64)
{
    return row.split('\t')
        .map(|x| x.parse::<u64>().unwrap())
        .fold((u64::max_value(), u64::min_value()), |(min, max), x| (cmp::min(min, x), cmp::max(max, x)));
}

fn solution1(input: &str) -> u64 {
    let rows = input.split('\n');
    let min_max = rows.map(min_max);
    return min_max.fold(0u64, |sum, (min, max)| sum + (max - min));
}
...
fn div2(row: &str) -> u64
{
    ...
    for i in 0..c.len() {
        for j in 0..c.len() {
            ...
            let (a, b) = if c[i] < c[j] { (c[j], c[i]) } else { (c[i], c[j]) };
            if a % b == 0 {
                return a / b;
            }
        }
